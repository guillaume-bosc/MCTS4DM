...
public class AssociationModelFactory implements UtilityModelFactory {
	...
	private List<AbstractFeature> getFeatures(DataTable dataTable) {
		...
		features.add(new ProductFeature(new FrequencyFeature(), deviation));

		// lift features
		AdditiveLiftFeature lift = new AdditiveLiftFeature(1.0);
		features.add(lift);
		features.add(new ProductFeature(new PowerFeature(
                new FrequencyFeature(), 0.5), lift, 1.0));
		features.add(new ProductFeature(new FrequencyFeature(), lift, 1.0));
	...
	public String getDescription() {
		return "Basic Features with deviation weights: 0.0, 0.0, 0.0;  lift weights: 1.0, 1.0, 1.0";
	}

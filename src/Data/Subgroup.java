...
public class Subgroup {
	...
	Pattern target; // Target of the subgroup

	public double measure; // Measure of this subgroup
	double e11; // Size of intersection between supportProp and supportTarg

	Map<Integer, Subgroup> children; // Array of children
	List<Subgroup> parent; // The parent of this subgroup
	...
	public boolean iterateOnce() {
		...
		if (newNode.measure > 0.)
			Global.allSubgroups.add(newNode);
	...
	public double rollOutRandomFrequentPath() {
		...
		if (Global.memoryPolicy == Enum.MemoryPolicy.AllEvaluated) {
			Global.allSubgroups.addAll(memoryList);
			...
			Global.allSubgroups.addAll(rankedMemory);
		}
		stopTime = System.currentTimeMillis();
	...
	public boolean isRedundantWith(Subgroup aSub) {
		...
		// labels
		if (this.target.sameTargets(aSub.target)) {
	...
	public void computeMeasure() {
		...
		if (Global.measure == Enum.Measure.F1)
			this.computeMeasureF1();
		else if (Global.measure == Enum.Measure.FBeta)
			this.computeMeasureFBeta();
		else if (Global.measure == Enum.Measure.WRAcc)
	...
	public void computeMeasureWRAcc() {
		...
		measure = (sizeG / sizeS) * (fracG - fracS);
		if (measure < 0)
			measure = 0;
	}
